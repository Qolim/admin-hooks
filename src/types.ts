...
export type SetRequestParamsStoreType = (
  requestParams:
    | GetPageDataRequestParamsStoreTypes
    | ((
        requestParams: GetPageDataRequestParamsStoreTypes
      ) => GetPageDataRequestParamsStoreTypes)
) => void;

export type SetLoadingType = (
  loading: boolean | ((l: boolean) => boolean)
) => void;
...
export type GetDataRequestType = (
  ...
  | Promise<any>
  | [
      (requestParams: GetPageDataRequestParamsStoreTypes) => Promise<any>,
      () => void
    ];

export type PageDataMapType = { tableData?: string; total?: string };

export type SetPageDataStoreType<T> = (
  pageData:
    | PageDataStoreType<T>
    | ((pageData: PageDataStoreType<T>) => PageDataStoreType<T>)
) => void;

export type ChangeRequestFiltersType = (
  f:
    | { [name: string]: any }
    | ((f: { [name: string]: any }) => { [name: string]: any })
) => void;

export type ChangeRequestPageType = (
  n: number | ((n: number) => number)
) => void;

export type AddNewRequestType<F> = (
  formData: F
) => Promise<any> | [Promise<any>, () => void];

export type UpdateTablePageDataType = () => void;

export type SetAddNewFormDataType<F> = (
  addNewFormDataStore: F | ((addNewFormDataStore: F) => F)
) => void;
